...
#폭발 시뮬레이션????
#중력은 입자들 사이에만 존재한다고 가정

#수정해야할 사항: 
#1.폭발 후 입자들이 충돌하지 않음
#2.drag 추가해서 가다가 속도 느려지도록 해야함
#3.초기 맨 처음 상태일때는 입자들이 한곳에 모여있어도 되는가?
#4.입자들간의 중력 추가해야함

#기본 셋팅
background_color = (255,255,255) #흰색
(width, height) = (1000, 1000)    #300X200
...
number_of_particles = 777 #입자의 갯수
...
for n in range(number_of_particles):
    ...
    x= 500 # x의 위치는 size와 (화면 넓이)-(size) 사이에서 임의의 값: 원이 화면밖으로 나가는것을 방지
    y= 500  #y도 x와 마찬가지 
